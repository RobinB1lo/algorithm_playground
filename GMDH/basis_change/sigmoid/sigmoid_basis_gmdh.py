...
class GMDHNeuronSigmoid:
    """Neuron with sigmoid basis functions instead of polynomials."""
    __slots__ = ('i', 'j', 'w', 'n_centers', 'k', 'a_min', 'a_max', 'b_min', 'b_max')

    def __init__(self, i: int, j: int, n_centers: int = 5, k: float = 8.0) -> None:
        self.i = i                          # column index
        self.j = j                          # column index
        self.n_centers = n_centers          # number of sigmoid centers per input
        self.k = k                          # sigmoid steepness parameter
    ...
    @staticmethod
    def _create_sigmoid_basis(x: np.ndarray, n_centers: int = 5, k: float = 8.0) -> tuple:
        """Create sigmoid basis functions for input x.
        ...
        # Evaluate each sigmoid basis function
        basis_functions = []
        for c in centers:
            basis_functions.append(1.0 / (1.0 + np.exp(-k * (x_norm - c))))

        return basis_functions, x_min, x_max, centers

    def _features(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        """Build features from sigmoid basis functions of a and b."""
        ...
        # Evaluate sigmoid basis functions
        a_basis = []
        b_basis = []
        for c in centers:
            a_basis.append(1.0 / (1.0 + np.exp(-self.k * (a_norm - c))))
            b_basis.append(1.0 / (1.0 + np.exp(-self.k * (b_norm - c))))
    ...
    def fit(self, a: np.ndarray, b: np.ndarray, y: np.ndarray,
            ridge: float = 1e-6) -> 'GMDHNeuronSigmoid':
        """Fit sigmoid coefficients via ridge regression."""
...
class GMDHLayerSigmoid:
    """GMDH layer using sigmoid basis functions."""

    def __init__(self, n_keep: int, ridge: float = 1e-6,
                 n_centers: int = 5, k: float = 8.0) -> None:
        self.n_keep = n_keep
        self.ridge = ridge
        self.n_centers = n_centers    # sigmoid centers per input
        self.k = k                    # sigmoid steepness
        self.neurons: List[GMDHNeuronSigmoid] = []
        self.best_error: float = np.inf

    def fit(self, Z_tr: np.ndarray, Z_se: np.ndarray,
            y_tr: np.ndarray, y_se: np.ndarray):
        """Fit layer: fit sigmoid neurons on train, score on selection."""
        m = Z_tr.shape[1]
        candidates = []

        for i, j in combinations(range(m), 2):
            nu = GMDHNeuronSigmoid(i, j, self.n_centers, self.k).fit(
...
class GMDH_Sigmoid:
    ...
    def __init__(self, n_keep: int, max_layers: int = 10, ridge: float = 1e-6,
        ...
        n_centers : int
            Number of sigmoid centers per input (default 5)
        k : float
            Sigmoid steepness parameter (default 8.0). Larger = sharper step.
        ...
        self.layers_: List[GMDHLayerSigmoid] = []
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for layer_num in range(self.max_layers):
            ...
            layer = GMDHLayerSigmoid(
                self.n_keep, self.ridge, self.n_centers, self.k
            )
