...
class GMDHNeuron:
    ...
    def aic(self, a: np.ndarray, b: np.ndarray, y: np.ndarray) -> float:
        """Calculate AIC (Akaike Information Criterion).
        
        AIC = 2*k + n*ln(RSS/n)
        where:
        - k = number of parameters (6 for the quadratic neuron)
        - n = number of samples
        - RSS = residual sum of squares
        
        Lower AIC is better. AIC balances fit quality with model complexity.
        """
        predictions = self.predict(a, b)
        residuals = y - predictions
        rss = np.sum(residuals ** 2)
        n = len(y)
        k = 6  # number of parameters in the quadratic polynomial
        aic_value = 2 * k + n * np.log(rss / n)
...
class GMDHLayer:
    ...
    def __init__(self, n_keep: int, ridge: float = 1e-6) -> None:
        self.n_keep = n_keep
        self.ridge = ridge
        self.neurons: List[GMDHNeuron] = []   # survivors, sorted best-first (lowest AIC)
        self.best_aic: float = np.inf         # lowest selection AIC in this layer
    ...
    def fit(self, Z_tr: np.ndarray, Z_se: np.ndarray,
        ...
        m = Z_tr.shape[1]
        # each entry: (selection_aic, neuron, train_output, selection_output)
        candidates = []

        for i, j in combinations(range(m), 2):
            nu = GMDHNeuron(i, j).fit(Z_tr[:, i], Z_tr[:, j], y_tr, self.ridge)
            aic_val = nu.aic(Z_se[:, i], Z_se[:, j], y_se)  # score on SELECTION using AIC
            out_tr = nu.predict(Z_tr[:, i], Z_tr[:, j])     # outputs to pass on
            out_se = nu.predict(Z_se[:, i], Z_se[:, j])
            candidates.append((aic_val, nu, out_tr, out_se))
...
class GMDH_AIC:
    """GMDH network with AIC-based selection: self-organizing polynomial network for regression.
    
    Uses Akaike Information Criterion (AIC) instead of RMSE to rank and select neurons.
    AIC balances model fit with complexity, preferring simpler models that fit equally well.
    """
    ...
    def __init__(self, n_keep: int, max_layers: int = 10, ridge: float = 1e-6,
        ...
        """Initialize GMDH network with AIC-based selection.
        
        Parameters:
        -----------
        n_keep : int
            Number of best neurons to keep per layer (by lowest AIC)
        max_layers : int
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        """Fit the GMDH network on data X and target y using AIC-based selection.
        ...
        for layer_num in range(self.max_layers):
            ...
            # Check if this layer improved (lower AIC is better)
